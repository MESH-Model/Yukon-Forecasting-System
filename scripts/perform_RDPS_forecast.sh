...
do
    ...
    output_file_path_gem="${run_file_path_gem}/${watershed}/${dt}16/RDPS"
	cd $output_file_path_gem
	
   # Obtain other mesh files that are unchanged from the common folder
	for file in `ls $home_dir/common/$watershed/*`
	do
		ln -sf $file ./
	done
	...
	for file in `ls $home_dir/template_files/$watershed/MESH*`
	do
		cp $file ./
	done
...
######################
    ...
    echo 'RDPS forecast MESH run complete for '$watershed' at' `date` >> $output_log_file
done
