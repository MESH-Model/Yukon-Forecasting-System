...
temp_file_path=$home_dir'/TempFiles/RDPS/'
...
do
    ...
    mkdir -p $run_file_path$watershed'/'$dt'16/RDPS'
 
    output_file_path=$run_file_path$watershed'/'$dt'16/RDPS/'
    ...
    do
      ...
      #  touch $scripts_file_path'header_info.txt'
        ...
        do
           ...
           f=$grib_file_path$FILES$dt$run_time'_P'$hour'*.grib2'
        ...
        gawk -f $scripts_file_path$'MESH_3.awk' $temp_file_path$names'_4.csv' > $temp_file_path$names'_5.csv'
	...
	rm $temp_file_path/*.* -f	
	echo 'RDPS processing complete for '$watershed' at' `date` >> $output_log_file
done
